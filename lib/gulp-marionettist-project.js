var browserSync, browserify, buffer, changed, coffeeify, concat, config, debowerify, fs, gulp, gulpIf, gutil, haml, hamlc, merge, path, pathmodify, plumber, sass, source, uglify, watch, watchify;
...
module.exports = function(options, autostart) {
  ...
  this.tasks = {
    ...
    sass: function() {
      ...
      return gulp.src(settings.sass.src).pipe(sass(settings.sass.options).on('error', sass.logError)).pipe(gulp.dest(settings.sass.dest)).pipe(browserSync.reload({
        stream: true
      }));
    },
    browserify: function() {
